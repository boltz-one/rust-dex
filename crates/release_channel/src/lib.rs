//! Provides constructs for the Boltz app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("BOLTZ_RELEASE_CHANNEL").unwrap_or_else(|_| {
            include_str!("../../boltz/RELEASE_CHANNEL")
                .trim()
                .to_string()
        })
    } else {
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Boltz-Dev",
        ReleaseChannel::Nightly => "Boltz-Nightly",
        ReleaseChannel::Preview => "Boltz-Preview",
        ReleaseChannel::Stable => "Boltz-Stable",
    }
}

/// The Git commit SHA that Boltz was built at.
...
impl AppVersion {
    ...
    pub fn load(
        ...
        let mut version: Version = if let Ok(from_env) = env::var("BOLTZ_APP_VERSION") {
            from_env.parse().expect("invalid BOLTZ_APP_VERSION")
        } else {
...
/// A Boltz release channel.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub enum ReleaseChannel {
    /// The development release channel.
    ///
    /// Used for local debug builds of Boltz.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for Boltz on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "dev.boltz.Boltz-Dev",
            ReleaseChannel::Nightly => "dev.boltz.Boltz-Nightly",
            ReleaseChannel::Preview => "dev.boltz.Boltz-Preview",
            ReleaseChannel::Stable => "dev.boltz.Boltz",
        }
    }
