...
fn is_called_from_gpui_crate(_span: Span) -> bool {
    ...
    // This is a heuristic approach - we check if the current crate name is the gpui crate
    // (published as `boltz-gpui`, historically `gpui`).
    std::env::var("CARGO_PKG_NAME").is_ok_and(|name| name == "gpui" || name == "boltz-gpui")
}
