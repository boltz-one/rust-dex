...
pub fn set_application_icon_png(bytes: &[u8]) -> Result<()> {
    if bytes.is_empty() {
        return Err(anyhow!("application icon PNG bytes are empty"));
    }

    unsafe {
        let pool = NSAutoreleasePool::new(nil);
        let data =
            NSData::dataWithBytes_length_(nil, bytes.as_ptr().cast::<c_void>(), bytes.len() as u64);

        if data == nil {
            pool.drain();
            return Err(anyhow!("failed to create NSData for application icon"));
        }
        ...
        if image == nil {
            pool.drain();
            return Err(anyhow!("failed to decode application icon PNG"));
        }
        ...
        unsafe {
            match item {
                ...
                MenuItem::Action {
                    ...
                    // later ones take display precedence.
