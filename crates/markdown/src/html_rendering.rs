//! `ui::prelude::*` (this crate cannot depend on `boltz-ui`; see
//! `controls.rs`'s module docs) is replaced by raw `gpui` +
//! `theme::ActiveTheme`. `Icon`/`IconButton`/`Label`/`Tooltip` aren't used by
//! this file (unlike `element.rs`), so no further replacement was needed.

use std::ops::Range;

use gpui::{
    App, FontStyle, FontWeight, InteractiveElement as _, IntoElement, ParentElement,
    StrikethroughStyle, Styled, TextAlign, TextStyleRefinement, UnderlineStyle, div,
...
use crate::parser::html::html_parser::{
...
mod tests {
    ...
    ) -> crate::rendered::RenderedText {
        ensure_theme_initialized(cx);

        let (_, cx) = cx.add_window_view(|_, _| TestWindow);
        let markdown = cx.new(|cx| Markdown::new(markdown.to_string().into(), cx));
        cx.run_until_parked();
