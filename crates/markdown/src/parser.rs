...
// `html` is nested under `parser` (not a crate-level module) because only
// `parser.rs` uses it — `html_rendering.rs` depends on `element.rs`/
// `builder.rs` instead. See `parser/html.rs` for details.
pub(crate) mod html;
