...
**Cập nhật (vòng 2, cùng ngày)**: user xác nhận qua `/goal` chấp nhận rủi ro dependency (giải quyết Unresolved Question #2) và yêu cầu hoàn thiện Phase C. Đã implement `crates/terminal` (PTY thật qua `alacritty_terminal` 0.25.1 — bản publish trên crates.io, không phải fork riêng của Zed) + `crates/ui/src/components/terminal_view.rs` (real shell, real I/O, verified bằng test thật + chạy `ui_gallery` thật, xác nhận qua `ps` không leak process). **Giới hạn còn lại, trung thực**: chỉ verify được macOS/Unix (môi trường phiên này không có Linux/Windows), chưa nối resize thật với `PaneGroup`, chưa có màu ANSI per-cell thật (chỉ text đơn sắc). Xem phase-03 § "Ghi chú triển khai thực tế" và § Success Criteria (đã đánh dấu mục nào đạt/chưa đạt) để biết chi tiết.
