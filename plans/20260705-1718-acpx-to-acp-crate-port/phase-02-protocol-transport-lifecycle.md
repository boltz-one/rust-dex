...
- **Implementation status:** Done, including the previously-deferred Gemini/Copilot/Devin runtime quirks (crate-wide `cargo test -p boltz-acpx --features test-support` now 274/274 pass: 259 lib unit tests + 6 `tests/client_lifecycle.rs` + 6 `tests/prompt_queue.rs` + 3 `tests/runtime_lifecycle.rs`), `cargo check -p boltz-acpx --all-targets --features test-support` clean (zero warnings), `cargo fmt -p boltz-acpx` clean, `make check-all` passes workspace-wide. All files < 200 lines. Deviation from ADR-1's original text: on inspecting the vendored SDK, all 3 "gap" methods (`session/set_mode`, `session/set_config_option`, `terminal/release`) turned out to already be fully typed in `agent-client-protocol-schema` — `jsonrpc_gap.rs` is a thin pass-through, not a hand-rolled implementation (see its module doc for detail). Agent-specific coverage per the resolved open question: Claude/Cursor/Codex/Gemini/Copilot/Devin detection predicates ported; Gemini version-probing/`--acp` rewrite, Copilot `--help` probing, and Devin's Windsurf identity-spoofing runtime quirks are now also ported — see `agent_command/gemini_quirks.rs`, `agent_command/copilot_quirks.rs`, `agent_command/command_probe.rs` (shared probe-subprocess helper), and `client/handshake.rs` (Devin identity, connection-time not command-line). Wired into both real spawn paths (`runtime/engine/manager_spawn.rs` and `runtime/public/probe.rs`) via new `SpawnAgentOptions.is_gemini`/`is_devin` fields on `client::AcpClient::spawn`, which races the Gemini `initialize` handshake against `resolve_gemini_acp_startup_timeout_ms()` and kills the hung subprocess + raises `AcpError::GeminiAcpStartupTimeout` on expiry. Minor deviations from acpx (documented in the new modules' doc comments): Copilot's unsupported-message builder reuses the already-fetched `--help` output instead of acpx's redundant second probe spawn; the message text is genericized away from the literal "acpx" CLI name since this is an embedded runtime, not a CLI.
- **Review status:** Not reviewed
- **Audit correction (2026-07-06):** a 4-way independent completeness audit found this phase's "done" status was inaccurate — several ported functions were never wired into the real call path. Specifically: gap 3 (`authenticate` RPC never called despite `auth_env.rs`'s credential-resolution helpers being fully implemented), gap 4 (Claude ACP's `session/new` startup-timeout — `AcpError::ClaudeAcpSessionCreateTimeout` defined but never constructed), gap 21 (Windows `.cmd`/`.bat` batch-shell wrapping absent from the real agent-spawn path in `client/spawn.rs`; `agent_command/spawn_options.rs`'s module doc comment falsely claims this logic already exists), gap 22 (Qoder's stdin-close-grace-period and benign-output-line-filter helpers orphaned; `buildQoderAcpCommandArgs` unported), gap 23 (legacy Claude model-metadata parsing never called on a live response), gap 26 (`codex_compat.rs`'s detection functions orphaned), gap 27 (`resolveClaudeCodeExecutable`, Windows-only, unported). All fixes tracked in `plans/20260706-0106-acp-completeness-fixes/`: gap 3 → [phase-01](../20260706-0106-acp-completeness-fixes/phase-01-permission-policy-authenticate-wiring.md), gap 4 → [phase-02](../20260706-0106-acp-completeness-fixes/phase-02-reconnect-hardening-claude-timeout.md), gap 23 → [phase-04](../20260706-0106-acp-completeness-fixes/phase-04-session-lifecycle-reconnect-model-state.md), gaps 21/27 → [phase-07](../20260706-0106-acp-completeness-fixes/phase-07-windows-batch-shell-spawn.md), gaps 22/26 → [phase-08](../20260706-0106-acp-completeness-fixes/phase-08-agent-quirks-shutdown-persistence.md). This phase's "done" status should be re-confirmed only once all of the above land.
...
13. `cargo fmt`, `cargo check -p boltz-acpx`, `make check-all`.
...
- [ ] Port `jsonrpc_gap.rs`.
- [x] Port `agent_command/*.rs`, `mcp_servers.rs`, `auth_env.rs`, `error_normalization.rs`, `error_shapes.rs`, `session_control_errors.rs`, `agent_session_id.rs`, `version.rs`. (Gemini/Copilot/Devin runtime quirks — the last remaining piece of `agent_command/*.rs` — landed in a follow-up pass: `agent_command/gemini_quirks.rs`, `agent_command/copilot_quirks.rs`, `agent_command/command_probe.rs`, plus `client/handshake.rs` for Devin's connection-time identity spoofing.)
...
- [ ] `cargo check -p boltz-acpx`, `make check-all`, `cargo fmt --all -- --check` green.
...
- ~~Unresolved question carried forward: **which agent-specific CLI variants ship first**~~ — Resolved: all six (Claude/Cursor/Codex/Gemini/Copilot/Devin) are now fully covered, including Gemini/Copilot/Devin's previously-deferred runtime quirks (version probing + `--acp` rewrite, `--help` capability pre-flight, Windsurf identity spoofing).
