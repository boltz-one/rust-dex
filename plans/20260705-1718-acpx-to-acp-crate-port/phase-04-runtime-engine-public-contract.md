...
- **Implementation status:** Done (2026-07-05). `runtime/{engine,public}/` implemented: public contract (`AcpRuntime` concrete struct, `AcpRuntimeHandle`/`Turn`/`Options`, `AcpSessionStore`/`AcpAgentRegistry` traits), event parsing over typed `SessionUpdate` (not raw-line parsing, see events.rs docs), reuse policy, session options, lifecycle helpers, connected-session, the reconnect state machine (explicit enum-driven acquisition path + replay-with-rollback), queue-agnostic prompt-turn, and the manager coordinator. Also extended Phase 2's `client/` (handshake handler wiring for fs/terminal/permission/session-update, plus `session_load`/`session_resume`/`prompt`/`cancel_session`/`set_session_mode`/`set_session_config_option` on `AcpClient`) and added `session::persistence::file_session_store::FileAcpSessionStore` (Phase 5-adjacent, per Implementation Step 11). `cargo fmt`, `cargo check -p boltz-acpx --all-targets --features test-support` (zero warnings), `cargo test -p boltz-acpx --features test-support` (239 tests: 233 unit + 6 integration, all green, including 2 pre-existing Phase 1-3/5 integration test files unmodified in behavior), and `make check-all` all pass. See implementation report for deviations (client-handler wiring done at handshake time rather than post-connect since the SDK has no post-connect handler API; `run_turn`'s per-turn token-usage persistence deferred since this schema build has no `PromptResponse.usage`; fake-agent binary extended with `session/prompt`+`session/update`+`session/resume`+a self-destruct env var for the crash-recovery test).
- **Review status:** Not reviewed
- **Audit correction (2026-07-06):** a 4-way independent completeness audit found this phase's "done" status was inaccurate, with the most serious finding being **zero unit tests** (`#[test]` count confirmed via grep) in `runtime/engine/reconnect/{mod.rs,replay.rs}` — this phase's own Risk Assessment called `reconnect.ts`'s 680-line port "the highest state-machine complexity in the whole scoped port," yet only 1 of ~6 `AcquisitionPath`-adjacent scenarios has any integration coverage (the Resume-succeeds happy path). Additional gaps: gap 6 (`prompt_turn`'s timeout handling doesn't check `has_agent_reply_after_prompt` before hard-failing, despite that exact function already being ported+tested with zero call sites); gap 9 (`session/close` RPC never sent to the backend agent — `close()` only does local cleanup); gap 12 (`SessionAgentOptions.model` persisted but never applied to a freshly created session — the pure logic in `session/model_application.rs` explicitly documents this phase as the intended caller); gap 13 (`get_capabilities()` always returns a static list, lost the per-session dynamism acpx's version has); gap 14 (`AcpRuntimeHandle.runtime_session_name` is a raw session-key copy — `handle_state.rs`'s encode/decode machinery is fully implemented and tested but has zero call sites); gap 15 (reconnect's post-replay model-state reconciliation functions don't exist in Rust at all — genuinely unported, not just unwired; `create_fresh_session` also leaves stale `config_options` uncorrected); gap 16 (`session/load` has no replay-suppression/drain, risking duplicate/out-of-order `session/update` events right after reconnect); gap 20 (`ClientOperation`/`onOperation` progress events incorrectly dismissed in `filesystem.rs`'s doc comment as "CLI-only, out of scope" — confirmed factually wrong, acpx's non-CLI `manager.ts` consumes this at the same tier as session-update handling); gap 33 (`reconnect/liveness.rs`'s `stored_process_status` is tested but orphaned — `manager/mod.rs` duplicates the concept with its own inline check); gap 34 (`ensure_session` doesn't validate blank `session_key`/`agent`); gap 35 (the `ConnectedSession` long-lived-client architecture, while a legitimate and correct simplification, was never formally documented as an ADR). All fixes tracked in `plans/20260706-0106-acp-completeness-fixes/`: gap 6 → [phase-02](../20260706-0106-acp-completeness-fixes/phase-02-reconnect-hardening-claude-timeout.md), gaps 9/12/15/16 → [phase-04](../20260706-0106-acp-completeness-fixes/phase-04-session-lifecycle-reconnect-model-state.md), gaps 13/14/34 → [phase-05](../20260706-0106-acp-completeness-fixes/phase-05-runtime-contract-dynamism.md), gap 20 → [phase-06](../20260706-0106-acp-completeness-fixes/phase-06-conversation-fidelity-client-operations.md), gap 33 → [phase-09](../20260706-0106-acp-completeness-fixes/phase-09-test-coverage-adr-cleanup.md), gap 35 → [phase-09](../20260706-0106-acp-completeness-fixes/phase-09-test-coverage-adr-cleanup.md) (ADR only, no code change). This phase's "done" status and its Todo list item "All new files < 200 lines" note should be re-confirmed only once all of the above land — the reconnect test-coverage gap in particular should be treated as this phase's single highest-priority follow-up.
