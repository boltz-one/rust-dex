...
- **Audit correction (2026-07-06):** a 4-way independent completeness audit found this phase's "done" status was inaccurate. Gap 7: `request_token_usage`'s eviction (`conversation_model/trim.rs`) uses a `std::collections::HashMap` with a `drain().take(N)` eviction — the code's own comment already admits this keeps an arbitrary N entries, not the N most-recently-inserted acpx's `Object.entries(...).slice(-100)` guarantees; needs `indexmap::IndexMap` (already a workspace dependency, just not yet added to this crate). Gap 8: `session/import/agent_match.rs`'s `assert_expected_agent_command` ANDs only 2 of acpx's 3 required conditions, omitting the `archiveAgentNameMatches` check entirely — this is exactly the kind of import-time trust-boundary gap this phase's own Security Considerations section warned against ("never trust import-time fields without re-validating"). Gap 17: tool-result `output` string fields are never truncated during runtime trim (only `content.Text` is). Gap 18: `to_raw_input` double-JSON-encodes string values (calls `.to_string()` on the whole `Value`, adding outer quotes acpx's `typeof === "string"` special-case avoids). Gap 19: `is_error`'s reset semantics diverge from acpx on partial tool-call updates (Rust's `Option<bool>` plumbing preserves a prior error flag across an update with no `status` field; acpx always writes a concrete boolean, resetting to `false`) — flagged by the audit as needing a deliberate choice (port acpx exactly vs. document as an accepted improvement), not left as an accidental divergence. Gap 28: `close_session` always sends SIGTERM then SIGKILL regardless of `last_agent_exit_signal`, unlike acpx's `killSignalCandidates`. Gap 29: `PruneOptions` is missing a convenience `older_than_ms` field. Gap 30: `model_control`'s legacy-migration default isn't backfilled at parse time (only compensated for at read time elsewhere). Gap 31: `trim_runtime_text` counts Unicode scalar values, not UTF-16 code units like acpx's `.slice()` — the doc comment's "matches acpx exactly" claim only holds for BMP-only text. All fixes tracked in `plans/20260706-0106-acp-completeness-fixes/`: gaps 7/8 → [phase-03](../20260706-0106-acp-completeness-fixes/phase-03-conversation-trim-import-security.md), gaps 17/18/19 → [phase-06](../20260706-0106-acp-completeness-fixes/phase-06-conversation-fidelity-client-operations.md), gaps 28/29 → [phase-08](../20260706-0106-acp-completeness-fixes/phase-08-agent-quirks-shutdown-persistence.md), gaps 30/31 → [phase-09](../20260706-0106-acp-completeness-fixes/phase-09-test-coverage-adr-cleanup.md).
...
- [x] `cargo check -p boltz-acpx`, `make check-all`, `cargo fmt --all -- --check` green (the workspace-wide `fmt-check` currently fails only in `crates/ui/...palette.rs`, owned by a concurrent, unrelated session — not this phase's files; `cargo fmt -p boltz-acpx -- --check` is clean).
